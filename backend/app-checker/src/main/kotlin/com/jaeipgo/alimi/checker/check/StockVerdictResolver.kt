...
class StockVerdictResolver {
    ...
    /**
     * @param expectedProductNo 이 스냅샷이 **어느 상품의 것이어야 하는지**.
     *   URL 에서 다시 뽑지 않고 호출자가 넘긴다 — 파싱은 등록 시점 한 곳에서만 한다
     *   (`core/product/SmartStoreUrl`). 그 값이 `StockCheckRequested` 에 실려 여기까지 온다.
     */
    fun resolve(snapshot: PageSnapshot, expectedProductNo: Long): CheckResult {
        ...
        if (actual != expectedProductNo) {
            return CheckResult.of(
                CheckOutcome.UNKNOWN,
                "상품 노드가 비었거나 다른 상품이다 (기대 $expectedProductNo, 실제 ${actual ?: "없음"})",
            )
        }
